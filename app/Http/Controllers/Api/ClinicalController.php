...
{
    /**
     * GET /api/get-patient-submited-form-data
     *
     * Fetches submitted form data for the authenticated patient from the external portal API.
     *
     * Request Payload:
     * - None
     *
    ...
    /**
     * POST /api/download-patient-submited-form-pdf
     *
     * Downloads external submitted form PDFs and returns them as a ZIP archive.
     *
     * Request Payload:
     * - pdfUrls (required, array<string>)
     *
     * Response:
     ...
     * - 500: { success: false, message: string }
     */
    public function getPatientFormData(Request $request)
    {
        try {
            Log::channel('patient_form')->info('Fetching grouped patient form data started', [
                'user_id' => auth()->id(),
            ]);
            ...
            // filter only when case_id is passed
            ->when($caseId, function ($query) use ($caseId) {
                $query->whereHas('userFunnel.patientCase', function ($q) use ($caseId) {
            ...
            foreach ($formSubmission as $item) {
                ...
                $currentCaseId = optional(optional($item->userFunnel)->patientCase)->case_id;
                ...
                if (!isset($groupedCases[$currentCaseId])) {
                    $groupedCases[$currentCaseId] = [
                ...
                $groupedCases[$currentCaseId]['cases']['forms'][] = $formData;
            }

            Log::channel('patient_form')->info('Grouped patient form data fetched successfully', [
                'user_id' => auth()->id(),
                'case_count' => count($groupedCases),
                'submission_count' => $formSubmission->count(),
            ]);
            ...
            Log::channel('patient_form')->error('Error fetching grouped patient form data', [
                'user_id' => auth()->id(),
                'error' => $e->getMessage(),
                'line' => $e->getLine(),
                'file' => $e->getFile(),
    ...
    /**
     * POST /api/download-patient-form-pdf
     *
     * Downloads local submitted form PDFs and returns them as a ZIP archive.
     *
     * Request Payload:
     * - pdfUrls (required, array<string>)
     *
